#!/usr/bin/env python3
"""
Gene Family Summary (Wide Format)
...
Output:
    - results/gene_family_summary_wide.csv
"""
...
OUTPUT_FILE = os.path.join(BASE_DIR, "results", "gene_family_summary_wide_sorghumversion3_recategorized.csv")
...
for family in all_families:
    ...
    sb_active = (sb["RecategorizeAction"] == "Activator").sum()
    zm_active = (zm["RecategorizeAction"] == "Activator").sum()

    sb_no_ortho_active = ((~sb["Has_Ortholog"]) & (sb["RecategorizeAction"] == "Activator")).sum()
    zm_no_ortho_active = ((~zm["Has_Ortholog"]) & (zm["RecategorizeAction"] == "Activator")).sum()
    ...
    sb_ortho_active = (sb_ortho_with_data & (sb["RecategorizeAction"] == "Activator")).sum()
    zm_ortho_active = (zm_ortho_with_data & (zm["RecategorizeAction"] == "Activator")).sum()

    # Of active genes, proportion that are orthologs vs non-orthologs
    # (uses Has_Ortholog regardless of partner data availability)
    sb_ortho_active_any = (sb["Has_Ortholog"] & (sb["RecategorizeAction"] == "Activator")).sum()
    zm_ortho_active_any = (zm["Has_Ortholog"] & (zm["RecategorizeAction"] == "Activator")).sum()
    ...
    gene_lists[family] = {
        ...
        "Sb_Active": sb.loc[sb["RecategorizeAction"] == "Activator", "gene_ID"].tolist(),
        "Zm_Active": zm.loc[zm["RecategorizeAction"] == "Activator", "gene_ID"].tolist(),
        "Sb_NoOrtho_Active": sb.loc[(~sb["Has_Ortholog"]) & (sb["RecategorizeAction"] == "Activator"), "gene_ID"].tolist(),
        "Zm_NoOrtho_Active": zm.loc[(~zm["Has_Ortholog"]) & (zm["RecategorizeAction"] == "Activator"), "gene_ID"].tolist(),
        ...
        "Sb_Ortho_Active": sb.loc[sb_ortho_with_data & (sb["RecategorizeAction"] == "Activator"), "gene_ID"].tolist(),
        "Zm_Ortho_Active": zm.loc[zm_ortho_with_data & (zm["RecategorizeAction"] == "Activator"), "gene_ID"].tolist(),
    }
...
OUTPUT_COUNTS = os.path.join(BASE_DIR, "results", "gene_family_summary_wide_counts_sorghumversion3_recategorized.csv")
...
for family_row in results:
    ...
    family_row["Sb_Active"] = (sb["RecategorizeAction"] == "Activator").sum()
    family_row["Zm_Active"] = (zm["RecategorizeAction"] == "Activator").sum()
...
OUTPUT_GENE_LISTS = os.path.join(BASE_DIR, "results", "gene_family_gene_lists_sorghumversion3_recategorized.pkl")
