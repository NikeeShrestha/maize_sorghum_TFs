#!/usr/bin/env python3
"""
Ortholog Pair Activity Visualization — RecategorizeAction column
=================================================================
For each ortholog pair (Sorghum–Maize), classifies the pair into one of
four activity categories based on the 'RecategorizeAction' column:
...
Produces:
  ...
  - results/ortholog_pair_activity_recategorized_proportions.csv

Notes:
  - Uses the 'RecategorizeAction' column (Activator = active, No Activity = inactive).
  - Only ortholog pairs where BOTH genes have RecategorizeAction data are included
    (NaN values are dropped).
  - Many-to-many orthologs are all included as separate pairs.

Output:
  results/ortholog_pair_activity_recategorized_proportions.csv
  results/ortholog_pair_activity_recategorized_stacked.png
"""
...
# ---- Load data ----
sorghum = pd.read_csv(SORGHUM_FILE)[["gene_ID", "family", "RecategorizeAction"]]
maize   = pd.read_csv(MAIZE_FILE)[["gene_ID", "family", "RecategorizeAction"]]
ortho   = pd.read_csv(ORTHOLOG_FILE)

# ---- Build ortholog-pair table ----
# Merge sorghum info
pairs = ortho.merge(
    sorghum.rename(columns={"gene_ID": "SorghumGene", "RecategorizeAction": "Sb_Action", "family": "family"}),
    on="SorghumGene", how="inner"
)
# Merge maize info
pairs = pairs.merge(
    maize.rename(columns={"gene_ID": "Ortholog", "RecategorizeAction": "Zm_Action"})[["Ortholog", "Zm_Action"]],
...
print(f"Total ortholog pairs with RecategorizeAction data: {len(pairs)}")
print(f"Unique families with data: {pairs['family'].nunique()}")

# ---- Canonical family order from Action pairs (matches Action plot) ----
# Build Action pairs just to derive the sorted family order
_sb_action = pd.read_csv(SORGHUM_FILE)[["gene_ID", "family", "Action"]]
_zm_action = pd.read_csv(MAIZE_FILE)[["gene_ID", "family", "Action"]]
_action_pairs = ortho.merge(
    _sb_action.rename(columns={"gene_ID": "SorghumGene", "family": "family", "Action": "_SbA"}),
    on="SorghumGene", how="inner"
).merge(
    _zm_action.rename(columns={"gene_ID": "Ortholog", "Action": "_ZmA"})[["Ortholog", "_ZmA"]],
    on="Ortholog", how="inner"
).dropna(subset=["_SbA", "_ZmA"])
_action_family_totals = _action_pairs.groupby("family").size().sort_values(ascending=False)
CANONICAL_FAMILY_ORDER = _action_family_totals.index.tolist()
print(f"Canonical family order: {len(CANONICAL_FAMILY_ORDER)} families")
...
# Reindex to canonical family order — fills 0 for families with no RecategorizeAction pairs
...
# Proportions (NaN where total == 0, i.e. no pairs for that family)
family_props = family_counts[CATEGORIES].div(family_counts["Total"].replace(0, float("nan")), axis=0).fillna(0)
...
# Use canonical family order (same as Action plot)
family_props_sorted  = family_props.loc[CANONICAL_FAMILY_ORDER]
family_counts_sorted = family_counts.loc[CANONICAL_FAMILY_ORDER]
