...
'''
This script identifies the overlap between DNA-binding domains (DBDs) and activation domains (ADs) in sorghum proteins.
It reads in protein data, filters for DNA-binding annotations, computes continuous regions of activation, and calculates the overlap between these regions and the DBDs.
...
unique_zma_proteins = pd.read_csv("../sorghumdata/sbi_proteins.iprscan", sep='\t',
...
overlapdf.to_csv('../results/sorghum_DD_AD_overlap_proteins.csv', index=False)

print(f"Total proteins with DD and AD overlap: {len(overlapdf)}")

print('file saved to ../results/sorghum_DD_AD_overlap_proteins.csv')
